#http://www.amazon.it/gp/aag/main?ie=UTF8&asin=&isAmazonFulfilled=&isCBA=&marketplaceID=A1F83G8C2ARO7P&orderID=&seller=A27JMYAV17VOL1&sshmPath=
#A27JMYAV17VOL1
#http://www.amazon.it/gp/aag/main?ie=UTF8&asin=&isAmazonFulfilled=&isCBA=&marketplaceID=A1F83G8C2ARO7P&orderID=&seller=A27JMYAV17VOL1&sshmPath=
#A27JMYAV17VOL1

#site="http://www.amazon.it/gp/aag/ajax/searchResultsJson.html"
site="https://www.amazon.it/gp/aag/ajax/getAsinListJson.html"
head="http://www.amazon.it/dp/"
...
function processline()
{
...

    {
      ...
      rank=`grep -a -o -zP "(id=\"SalesRank\")[^\(]+\d+[^\(]+(?=\()" $filename | tail -n 1 | grep -a -o -zP "\d+.*"`
      ...
      new_sellor=`grep -a -o -zP "(?<=condition=new\")([\s\S]+?)</span" $filename | grep -a -o -zP "(?<=>)[\s\S]+(?=</span)" | head -n 1 | awk -vFS=">|<" '{print $1" "$3" "$5}'`
      old_sellor=`grep -a -o -zP "(?<=condition=used\")([\s\S]+?)</span" $filename | grep -a -o -zP "(?<=>)[\s\S]+(?=</span)" | head -n 1 | awk -vFS=">|<" '{print $1" "$3" "$5}'`
